...
class Mycallback: public GRBCallback {
	...
	Mycallback(GRBVar **xvars, int _r, int qnd_n, double **_matriz_custo,
	           ...
	           double _prize, double _prize_V, double _alvo) {
		vars = xvars;
		n = qnd_n;
...
//			}
	...
	bool cut_set9(bool debug, int tipo) {
		if (debug)
			printf(
			    "\n\n-------------- MIPSOL (9) ----------------------------------------------------\n");
		...
		//
		//
		////			cout << varsY[i].get(GRB_StringAttr_VarName) <<" = " << varsY[i].get(GRB_DoubleAttr_X) << " \t";
		//			if (debug)
		//				printf("y[%d] = %.2f \t", i, y[i]);
		//		}
		//
		...
		for (i = 0; i < n; i++) {
			if (sol[i] != 1 || i == r || Vertexs[i].tipo != 0) {
				continue;
			}

			//i \in R de cobrir pelo menos algum w \in W
			if (Vertexs[i].cobre.size() < 1) {
				continue;
			}
		...
		try {
			...
			case GRB_CB_MIPNODE:
				if (getIntInfo(GRB_CB_MIPNODE_STATUS) == GRB_OPTIMAL) {
					...
					if (do_cuts) {
						kruskal2Matching(false);

//							lifted_cover_GENDREAU();
//						getchar();
						if (!cover_cut(false, 1)) {
							cover_cut(false, 2);
						}
...
//						getchar();
			...
			case GRB_CB_MIPSOL:
//				if (cut_set_MIPSOL_7(false)) {
//					break;
//				}

				cut_set_MIPSOL_7(false);
//				if (cut_set_MIPSOL_8(false))
//					break;

//				if (cut_set_MIPSOL_8(false))
//					break;
				//				cut_set_MIPSOL_8(false);
