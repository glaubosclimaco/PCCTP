...
/* Gera string com data e hora para nome de ficheiro: YYYY-MM-DD_HH-MM-SS */
...
/* Cria resultados/<metodo>/ e grava conteudo em resultados/<metodo>/<timestamp>.txt */
static void salvar_resultado(const char* metodo, const string& conteudo) {
	mkdir("resultados", 0755);  /* ignora erro se ja existir */
	string dir = string("resultados/") + metodo;
	if (mkdir(dir.c_str(), 0755) == -1 && errno != EEXIST) return;
	string path = dir + "/" + get_timestamp() + ".txt";
	ofstream f(path);
	if (f.is_open()) {
		f << conteudo;
		f.close();
...
double solver(double perfix, vector<Vertex>& vSol, vector<aresta>& L, int delta, int fazLB,
              ...
              int qtd_t, int qnd_vt, int qtd_n, vector<Vertex> vertices,
...
//	Nome do arquivo de instancia PRRCP
	...
	/* modo: 0 = GRASP + solver (MIP); 1 = so heuristica (GRASP e termina) */
	int modo = 0;
	if (argc != 9 && argc != 10) {
		printf(
		    "Uso: %s <instancia> <seed> <aplica_cortes> <remove_E0> <gamma> <aplica_regras> <ordem> <perfix> [modo]\n",
		    argv[0]);
		printf("  modo: 0 = solver (GRASP + MIP), 1 = so heuristica (GRASP)\n");
		return (-1);
	}
	if (argc == 10)
		modo = atoi(argv[9]);
...
//
		char buf_regras[512];
		snprintf(buf_regras, sizeof(buf_regras), "%s \t %d \t %d \t %d \t %d \t %.2f\n",
		         file_name, rReduzidos, tReduzidos, wReduzidos, totalReducoes, tempo_total);
		printf("%s \t %d \t %d \t %d \t %d \t", file_name, rReduzidos,
		       tReduzidos, wReduzidos, totalReducoes);
		printf("%.2f \n", tempo_total);
		salvar_resultado("regras", buf_regras);

		return 1;  /* termina apos regras de reducao */
	}

	/* Modo so heuristica: termina apos GRASP (requer aplicaE=1) */
	if (modo == 1) {
		char buf_grasp[512];
		snprintf(buf_grasp, sizeof(buf_grasp), "Heuristica: %s \t otimo= %.0f \t custo= %.0f \t tempo_GRASP= %.2f\n",
		         file_name, valor_otimo, custo_obtido, tempo_grasp);
		printf("%s", buf_grasp);
		salvar_resultado("grasp", buf_grasp);
		return (0);
	}

	/*
	 * SOLVER: redireciona stdout/stderr para ficheiro de log (logs Gurobi + linha MIP)
	 */

	const char* metodo_mip = aplicaE ? "hibrido" : "solver";
	mkdir("resultados", 0755);
	string dir_mip = string("resultados/") + metodo_mip;
	if (mkdir(dir_mip.c_str(), 0755) == -1 && errno != EEXIST) { }
	string logpath = dir_mip + "/" + get_timestamp() + ".txt";
	...
	char buf_mip[512];
	snprintf(buf_mip, sizeof(buf_mip), "MIP: %s \t %.0f \t %.0f \t %.2f\n",
	         file_name, valor_otimo, custo_solver, tempoMPO);
	printf("%s", buf_mip);
	fflush(stdout);

	if (flog && saved_stdout >= 0) {
...
/* Sobrecarga sem perfix: chamada por main(); delega ao solver completo com perfix=0 */
double solver(vector<Vertex>& vsol, vector<aresta>& L, int delta, int fazLB, vector<edgeSol>& solucao,
              int** matriz_BL, bool removeE0, int root, int qtd_t, int qnd_vt, int qtd_n,
              vector<Vertex> vertices, double **matriz_custo, double prize, bool exigir_r, double prize_V,
              double otimo, double limPrimal) {
	return solver(0.0, vsol, L, delta, fazLB, solucao, matriz_BL, removeE0, root,
	              qtd_t, qnd_vt, qtd_n, vertices, matriz_custo, prize, exigir_r, prize_V,
	              otimo, limPrimal);
}
...
//		}
		...
		model.getEnv().set(GRB_IntParam_OutputFlag, 1);  /* 1 = mostrar log da execucao do solver */
		/* SolutionLimit=1 faz o solver parar na 1.a solucao; removido para buscar o otimo (ou ate TimeLimit) */
